...
- The generated Assembly Front SVG dimensions total enclosure height on the left and current base height on the right. Integration coverage verifies regenerated `36.00/21.00` and `37.00/25.00` value pairs, and the right-side label remains inside the SVG viewport.
