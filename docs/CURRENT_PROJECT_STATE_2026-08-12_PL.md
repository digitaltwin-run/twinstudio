...
| Pełny pytest/CI | lokalnie zielony | 111/111 testów przechodzi z `httpx2`; workflow instaluje pełne zależności obu projektów |
...
- kontekstowe polecenia bez nazwy parametru, np. `obniż do 14 mm`, są wiązane z fizyczną wysokością zaznaczonego komponentu; przed `ChangeApplied` runtime uruchamia synchroniczny CAD preflight, dlatego niepoprawne `obniż do 12 mm` dla bieżącej pokrywy zwraca typowany problem `CAD-CHANGE-INVALID` z ograniczeniem `AUX_BOSS_TOP_ABOVE_LID`, ale nie zmienia rewizji, parametru ani artefaktów;
...
- pełny pytest: **111/111 passed**, łącznie z testami API przez FastAPI `TestClient`, macierzą prostych i kontekstowych poleceń NL, preflightem niepoprawnej geometrii, ścisłą granicą propozycji LLM, odrzuceniem niezgodnej odpowiedzi jako jawnego `LLM-INVALID-RESPONSE`, kontrolą zakresu POA, runtime authority receipt, prawdziwą regeneracją i ponowną regeneracją po cofnięciu przez CadQuery, kontraktem `ProblemEnvelope`/`UIContext`, eksportem logów TWINOBS i PDF każdej zakładki, kontrolą procesu lokalnego, mapowaniem zaznaczenia SVG 2D, stanem oczekiwania planera, historią/cofaniem parametrów i kolejnością `.env.local`/`.env`;
