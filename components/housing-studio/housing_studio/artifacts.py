...
def _write_bom(path: Path, config: ProjectConfig) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["item", "part_number", "description", "quantity", "source", "notes"])
        writer.writerow([1, "BASE-001", "Lower housing base", 1, "generated", "Print-ready CAD part"])
        writer.writerow([2, "LID-001", "Upper hinged lid", 1, "generated", "Print-ready CAD part"])
        writer.writerow(
            [
                3,
                "PIN-REF",
                ...
                "external hardware",
                (
                    f"Nominal diameter {config.hinge.pin_diameter:.2f} mm; "
                    "final material and retention method require confirmation"
                ),
            ]
        )


def _rebuild_script(config: ProjectConfig) -> str:
    config_literal = repr(config.model_dump_json(indent=2))
    return (
        "#!/usr/bin/env python3\n"
        '"""Rebuild this housing revision from its embedded validated configuration.\n\n'
        "Run from an installed copy of the Housing Studio project.\n"
        '"""\n'
        "from __future__ import annotations\n\n"
        "import argparse\n"
        "from pathlib import Path\n\n"
        ...
        f"CONFIG_JSON = {config_literal}\n\n"
        "def main() -> int:\n"
        "    parser = argparse.ArgumentParser(description='Rebuild the packaged housing revision')\n"
        "    parser.add_argument('--out', type=Path, default=Path('regenerated'))\n"
        "    parser.add_argument('--job-id', default='rebuilt-housing')\n"
        "    args = parser.parse_args()\n"
        "    config = ProjectConfig.model_validate_json(CONFIG_JSON)\n"
        "    manifest = generate_artifacts(config, args.out, job_id=args.job_id)\n"
        "    print((args.out.resolve() / manifest['job_id']).as_posix())\n"
        "    return 0\n\n"
...
def _generated_readme(config: ProjectConfig) -> str:
    return "\n".join(
        [
            "# Generated housing revision",
            "",
            f"Project: **{config.metadata.name}**  ",
            f"Revision: **{config.metadata.revision}**  ",
            f"Generator: **Housing Studio {GENERATOR_VERSION}**  ",
            f"Units: **{config.metadata.units.value}**",
            "",
...
def _change_report_markdown(
    changes: list[dict[str, Any]],
    *,
    source_prompt: str | None,
    interpretation_mode: str | None,
) -> str:
    lines = ["# Configuration change report", ""]
    if interpretation_mode:
        lines.extend([f"Interpretation mode: **{interpretation_mode}**", ""])
    if source_prompt:
        lines.extend(["## Natural-language request", "", source_prompt.strip(), ""])
    lines.extend(["## Applied changes", ""])
    if not changes:
        lines.append("No configuration values changed.")
    else:
        for change in changes:
            path = str(change.get("path", "$"))
            kind = str(change.get("kind", "changed"))
            before = repr(change.get("before"))
            after = repr(change.get("after"))
            if len(before) > 160:
                before = before[:157] + "..."
            if len(after) > 160:
                after = after[:157] + "..."
            lines.append(f"- `{path}` — **{kind}**: `{before}` → `{after}`")
    lines.extend(
        [
            "",
            "This report records configuration changes. It does not replace dimensional inspection or a physical prototype.",
            "",
        ]
    )
    return "\n".join(lines)
...
def generate_artifacts(
    ...
    *,
    ...
    configuration_changes: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    rebuild_path = job_dir / "rebuild_project.py"
    rebuild_path.write_text(_rebuild_script(config), encoding="utf-8")
    rebuild_path.chmod(0o755)
    records.append(
        _record(job_dir, rebuild_path, "source", "Reproducible Python rebuild script", "text/x-python")
    )

    generated_readme_path = job_dir / "README_GENERATED.md"
    generated_readme_path.write_text(_generated_readme(config), encoding="utf-8")
    records.append(
        _record(job_dir, generated_readme_path, "documentation", "Generated revision readme", "text/markdown")
    )

    bom_path = job_dir / "bill_of_materials.csv"
    _write_bom(bom_path, config)
    records.append(_record(job_dir, bom_path, "documentation", "Bill of materials", "text/csv"))
    ...
    if configuration_changes is not None:
        changes_path = job_dir / "configuration_changes.json"
        _write_json(
            changes_path,
            {
                "interpretation_mode": interpretation_mode,
                "change_count": len(configuration_changes),
        ...
        records.append(
            _record(
                ...
                changes_path,
                "configuration",
                "Auditable natural-language configuration changes",
                "application/json",
            )
        )

        change_report_path = job_dir / "configuration_changes.md"
        change_report_path.write_text(
            _change_report_markdown(
        ...
        records.append(
            _record(
                ...
                change_report_path,
                "documentation",
                "Human-readable configuration change report",
                "text/markdown",
            )
        )
    ...
    manifest: dict[str, Any] = {
        ...
        "interpretation": {
            ...
            "change_count": len(configuration_changes or []),
            "source_prompt_included": bool(source_prompt),
        },
        ...
        "bundle": bundle_name,
        # The manifest and ZIP deliberately do not hash themselves. Every listed artifact
        # is independently verifiable, while manifest.json remains byte-identical inside
        # and outside the ZIP bundle.
