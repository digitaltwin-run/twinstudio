# Housing Studio 1.2.1 - generator obudowy 2D/3D
...
- dokumentację PDF,
- pliki JSON konfiguracji, warstw, metryk, ostrzeżeń i zmian rewizji,
- raport zmian w Markdown, BOM w CSV i samodzielny skrypt `rebuild_project.py`,
- kompletną paczkę ZIP,
- lokalny interfejs webowy z podglądem 3D, warstwowym podglądem SVG, historią rewizji i pobieraniem plików.

LiteLLM jest używany jako bezpieczny „kompilator konfiguracji”: opis po polsku lub angielsku jest przekształcany w kompletny obiekt `ProjectConfig`, a następnie walidowany przez Pydantic. Interfejs pokazuje dokładny diff wartości i wymaga jawnego zatwierdzenia zmian. Model językowy nie generuje ani nie wykonuje kodu CAD.
...
├── housing_studio/
│   ├── version.py                # wspólna wersja API/generatora
│   ├── models.py                 # schemat całej konfiguracji
│   ├── llm_config.py             # LiteLLM + parser awaryjny
│   ├── config_diff.py            # audytowalny diff konfiguracji
...
├── docs/                           # specyfikacja, architektura, changelog i weryfikacja
├── tests/
├── Dockerfile
└── docker-compose.yml
```
...
Podgląd 3D w przeglądarce ładuje moduły Three.js z jsDelivr. Jeżeli przeglądarka nie ma dostępu do CDN, aplikacja nadal uruchamia edytor konfiguracji, LiteLLM/parser lokalny, dokumentację 2D, generowanie i pobieranie plików; niedostępny pozostaje wyłącznie interaktywny podgląd STL. Dostęp do internetu jest ponadto potrzebny do wywołania zewnętrznego modelu przez LiteLLM.
...
## Przepływ pracy w interfejsie webowym

1. Wpisz zmianę po polsku lub angielsku.
2. LiteLLM albo parser lokalny tworzy konfigurację kandydującą.
3. Panel **Przegląd zmian** pokazuje każdą zmienioną ścieżkę, wartość wcześniejszą i nową.
4. Zatwierdź lub odrzuć propozycję.
5. Dostosuj kluczowe wymiary, warstwy 3D, warstwy 2D, widoki oraz formaty za pomocą przełączników.
6. Waliduj konfigurację i wygeneruj rewizję.
7. Otwieraj poprzednie rewizje z historii i używaj ich jako bazy kolejnych zmian.

Podgląd SVG zawiera grupy oznaczone kluczami warstw. Można je ukrywać i pokazywać bez regeneracji samego podglądu; pliki DXF i PDF odzwierciedlają stan warstw z chwili generowania.
...
Pakiet zawiera 27 testów obejmujących schemat, parser lokalny, audyt zmian, most LiteLLM z odpowiedzią zgodną z JSON Schema, bryły CadQuery, szczelność siatek STL, warstwy DXF i SVG, skrypt odtwarzania rewizji, sumy SHA-256, zgodność manifestu w ZIP oraz endpointy FastAPI. Szczegóły znajdują się w `docs/VERIFICATION.md`.
...
`manifest.json` zawiera rozmiar i sumę SHA-256 każdego wymienionego artefaktu. Manifest i paczka ZIP nie próbują haszować samych siebie; dzięki temu finalny manifest w katalogu zadania jest bajtowo identyczny z kopią wewnątrz `housing_project_bundle.zip`.

Historia zmian wydania znajduje się w `docs/CHANGELOG.md`.
