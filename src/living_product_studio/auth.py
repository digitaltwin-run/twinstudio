...
from living_product_studio.domain import AuthPrincipal, CommandEnvelope, EventEnvelope, InvitationRequest, Role
from living_product_studio.event_store import EventStore
from living_product_studio.settings import Settings

if TYPE_CHECKING:
    from living_product_studio.mqtt_bus import EventPublisher
...
class AuthService:
    ...
    def principal_from_request(self, request: Request) -> AuthPrincipal:
        ...
        session = request.cookies.get("lps_session")
        if session:
        ...
        raise HTTPException(
            ...
            headers={"WWW-Authenticate": "Basic realm=LivingProductStudio"},
        )
